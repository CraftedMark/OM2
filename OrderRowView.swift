    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Image(systemName: order.orderTotal?.compare(order.paidAmount ?? <#default value#>) == .orderedSame ? "checkmark.circle.fill" : "xmark.circle.fill")
...
struct OrderRowView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        return OrderRowView(order: sampleOrder)
            ...
            .padding()
            .environment(\<#Root#>.managedObjectContext, context)
    }
}
